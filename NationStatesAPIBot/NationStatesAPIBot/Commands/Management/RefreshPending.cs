...
namespace NationStatesAPIBot.Commands.Management
{
    public class RefreshPending : ModuleBase<SocketCommandContext>
    {
        ...
        {
            await ActionManager.LoggerInstance.LogAsync(Discord.LogSeverity.Debug, "RefreshPending - HandleNew", "Entered");
            var result = await ActionManager.NationStatesApiController.RequestNewNationsAsync(false);
            await ActionManager.LoggerInstance.LogAsync(Discord.LogSeverity.Debug, "RefreshPending - HandleNew", "Nations requested");
            var newnations = ActionManager.NationStatesApiController.MatchNationsAgainstKnownNations(result, "pending");
            await ActionManager.LoggerInstance.LogAsync(Discord.LogSeverity.Debug, "RefreshPending - HandleNew", "Nations matched");
            await ActionManager.NationStatesApiController.AddToPending(newnations);
            await ActionManager.LoggerInstance.LogAsync(Discord.LogSeverity.Debug, "RefreshPending - HandleNew", $"{newnations.Count} Added to Pending - done");
