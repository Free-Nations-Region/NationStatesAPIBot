...
namespace NationStatesAPIBot.Commands.Stats
{
    ...
    {
        [Command("nation"), Summary("Returns Basic Stats about a specific nation")]
        public async Task GetBasicStats(string nationName)
        {
            try
            {
                await ActionManager.LoggerInstance.LogAsync(LogSeverity.Verbose, "BasicNationStats", $"BasicNationStats for {nationName} requested.");
                ...
                using (var stream = await ActionManager.NationStatesApiController.ExecuteRequestAsync(request, NationStatesApiRequestType.GetNationStats))
                {
                    if (stream != null)
                    {
                        ...
                        builder.AddField(wa, $"{endorsementCount} endorsements | {influenceValue} Influence ({Influence})");
